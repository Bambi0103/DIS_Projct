...
if [[ -z $role_exists ]]; then
    ...
    if [[ $create_role == [Yy] ]]; then
        read -r "-s -p 'Enter password for new role: ' new_role_password
        ...
        psql -U postgres -c "CREATE ROLE $curr_user WITH LOGIN SUPERUSER PASSWORD '$curr_user';"
        if [ $? -ne 0 ]; then
            echo "Failed to create role '$curr_user'. Please check your PostgreSQL configuration."
            exit 1
        else
            echo "Role '$curr_user' created successfully."
        fi
    ...
    echo "User role '$curr_user' exists in PostgreSQL."
fi

psql_output=$(psql -U "$curr_user" -f db_files/create-db.sql 2>&1)
psql_exit=$?
...
if [ $psql_exit -ne 0 ]; then
    if echo "$psql_output" | grep -q "already exists"; then
        ...
        echo "Failed to create the database. Error output:"
        echo "$psql_output"
    ...
    echo "Database 'guesstheplayerdb' created successfully."
fi
...
try_cmd() {
    # $1 … full command (can contain spaces, so we eval)
...
if [[ -z $PYTHON_CMD ]]; then
    echo "❌  No Python 3.13.x interpreter found in PATH." >&2
    exit 1
fi

python_version="$($PYTHON_CMD --version 2>&1)"
echo "✅  Found $python_version (via '$PYTHON_CMD')."
...
if [[ -f venv/bin/activate ]]; then
    ...
    # Windows layout (works in Git Bash, MSYS, Cygwin, WSL)
    source venv/Scripts/activate
else
    echo "❌  Could not find an 'activate' script inside venv." >&2
    exit 1
fi
...
if [[ $run_choice == [nN] ]]; then
    ...
    if [ $? -ne 0 ]; then
        echo "Failed to run main.py. Please check your configuration."
        exit 1
    else
